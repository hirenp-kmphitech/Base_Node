...
const sendSMS = async function (phoneNumber) {
    if (commonConfig.IS_TESTING == "true") return { response: "test sms", status: true };
...
const verifySMSCode = async function (phoneNumber, code) {
    if (commonConfig.IS_TESTING == "true") return { response: "test sms", status: ((commonConfig.TESTING_OTP == code) ? true : false) };
